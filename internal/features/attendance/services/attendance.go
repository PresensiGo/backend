...
type Attendance struct {
	db                *gorm.DB
	attendance        *repositories2.Attendance
	attendanceStudent *repositories2.AttendanceDetail
...
func (s *Attendance) Create(req requests.CreateAttendance) error {
	if err := s.db.Transaction(
		func(tx *gorm.DB) error {
			...
			mappedAttendanceStudents := make(
				[]domains.AttendanceDetail, len(req.AttendanceStudents),
			)
...
func (s *Attendance) GetById(attendanceId uint) (*responses.GetAttendance, error) {
	...
	mapAttendanceStudents := make(map[uint]domains.AttendanceDetail)
